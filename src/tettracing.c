...
float ray_sphere_intersect(ray *r, raytracer *tracer, int *ee, int index, float nr){
	...
	if((npdist0>nr2+EPS2 && npdist1<nr2-EPS2) || (npdist0<nr2-EPS2 && npdist1>nr2+EPS2)){
		r->isvessel = 2;
		oo = r->p0;
		ll = r->vec;
		vec_diff(&cc,&oo,&oc);
...
float branchless_badouel_raytet(ray *r, raytracer *tracer, mcconfig *cfg, visitor *visit){
	...
	if(r->faceid>=0 && bary.x>=0){
		...
		hit = ray_cylinder_intersect(r, tracer, ee, 1);
	    	...
	    	if(neweid<0){	// avoid missing the face while the current element does not have the label
	    		noface = 1;
	    		neweid = -neweid;
	    		r->faceeid = neweid;
